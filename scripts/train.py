...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  # Add wandb configuration for better organization by task
  if "wandb_group" not in agent_cfg:
    agent_cfg["wandb_group"] = task_id
  
  # Set or update wandb_tags - replace empty tags with task_id
  existing_tags = agent_cfg.get("wandb_tags", [])
  if not existing_tags or (isinstance(existing_tags, (list, tuple)) and len(existing_tags) == 0):
    agent_cfg["wandb_tags"] = [task_id]
  
  # Set wandb environment variables to ensure they are picked up by wandb.init()
  # This is a workaround for RSL-RL not passing these parameters from config to wandb.init()
  if agent_cfg.get("logger") == "wandb":
    if "wandb_group" in agent_cfg and agent_cfg["wandb_group"]:
      os.environ["WANDB_RUN_GROUP"] = agent_cfg["wandb_group"]
    
    tags = agent_cfg.get("wandb_tags", [])
    if tags and isinstance(tags, (list, tuple)):
      # Filter out empty strings and convert to comma-separated string
      valid_tags = [str(t) for t in tags if t]
      if valid_tags:
        os.environ["WANDB_TAGS"] = ",".join(valid_tags)
  ...
  if rank == 0:
    ...
    try:
      ...
      # Upload deploy.yaml to wandb if using wandb logger
      if agent_cfg.get("logger") == "wandb":
        deploy_yaml_path = log_dir / "params" / "deploy.yaml"
        if deploy_yaml_path.exists():
          import wandb
          if wandb.run is not None:
            wandb.save(str(deploy_yaml_path), policy="now")
            print(f"[INFO] Uploaded deploy.yaml to WandB")
