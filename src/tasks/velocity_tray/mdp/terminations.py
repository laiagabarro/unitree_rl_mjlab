...
def cube_fallen(
    ...
    """Terminate when a cube falls, with probability increasing with curriculum progress."""
    ...
    for cube_name in cube_names:
        ...
        tray_pos = tray.data.root_link_pos_w
        tray_quat = tray.data.root_link_quat_w
        tray_quat_inv = quat_inv(tray_quat)

        half_size = cube.data.root_link_pos_w.new_tensor(
            cube.cfg.init_state.pos[2]
        )
        ...
        at_resting_height = (
            torch.abs(local_pos[:, 1] - half_size) < height_threshold
        )
...
def is_terminated_without_cube_fall(
    env: ManagerBasedRlEnv,
) -> torch.Tensor:
