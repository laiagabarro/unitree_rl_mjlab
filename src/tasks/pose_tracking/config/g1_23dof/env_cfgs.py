"""Unitree G1-23DOF pose tracking environment configurations."""
...
from src.tasks.pose_tracking import mdp
from src.tasks.pose_tracking.mdp import UniformPose2dCommandCfg
from src.tasks.pose_tracking.pose_tracking_env_cfg import make_pose_tracking_env_cfg


def unitree_g1_23dof_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Unitree G1-23DOF rough terrain pose tracking configuration."""
  cfg = make_pose_tracking_env_cfg()
  ...
  pose_cmd = cfg.commands["pose_2d"]
  assert isinstance(pose_cmd, UniformPose2dCommandCfg)
  pose_cmd.viz.z_offset = 0.45
...
def unitree_g1_23dof_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Unitree G1-23DOF flat terrain pose tracking configuration."""
  ...
  if play:
    pose_cmd = cfg.commands["pose_2d"]
    assert isinstance(pose_cmd, UniformPose2dCommandCfg)
    pose_cmd.ranges.pos_x = (-0.5, 0.5)
    pose_cmd.ranges.pos_y = (-0.5, 0.5)
    pose_cmd.ranges.heading = (-3.14, 3.14)
