...
def phase(env: ManagerBasedRlEnv, period: float, command_name: str) -> torch.Tensor:
    ...
    # For pose commands, check if error is small (robot is near target)
    command = env.command_manager.get_term(command_name)
    # Command object should have .error property for pose commands
    stand_mask = torch.linalg.norm(command.error, dim=1) < 0.1
