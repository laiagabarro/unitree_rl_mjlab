...
public:
    ...
    void step()
    {
        episode_length += 1;
        robot->update();
        if (post_update_hook) post_update_hook();
        auto obs = observation_manager->compute();
        auto action = alg->act(obs);
        action_manager->process_action(action);
    }

    /// Optional callback invoked after robot->update() and before observation
    /// computation. Use it to patch robot->data.joint_pos/joint_vel.
