...
struct ArticulationData
{
    ...
    // Command velocity override (for PID controllers that compute commands from goal pose)
    // When non-null, this overrides joystick values for velocity observations
    Eigen::Vector3f* command_velocity = nullptr;  // [vel_x, vel_y, vel_yaw]
};
