...
namespace unitree
{
    ...
    public:
        ...
        BaseArticulation(LowStatePtr lowstate_, isaaclab::GoalPoseSubscription::SharedPtr goal_pose_sub_)
            : lowstate(lowstate_), goal_pose_sub(goal_pose_sub_)
        {
            data.joystick = &lowstate->joystick;
            if (goal_pose_sub)
            {
                data.goal_pose = &goal_pose_sub->data();
            }
        }

        /**
         * @brief Set the goal pose subscription after construction
         */
        void setGoalPoseSubscription(isaaclab::GoalPoseSubscription::SharedPtr goal_pose_sub_)
        {
            goal_pose_sub = goal_pose_sub_;
            if (goal_pose_sub)
            {
                data.goal_pose = &goal_pose_sub->data();
            }
